...
class Controller2D(object):
    def __init__(self, waypoints, control_method):
        ...
        self._wheelbase          = 3.0  # 纯轨迹跟踪使用
        self._control_method     = control_method
        self._steering_diff      = np.linspace(-40, 40, 161, endpoint=True)

    def update_values(self, x, y, yaw, speed, timestamp, frame, closest_distance, desired_speed):
        self._current_x         = x
        self._current_y         = y
    ...
    def calculate_throttle(self, t, v, v_desired):
        ...
        i_term = 0.5 * self.vars.i_term_previous + self._kI*time_step*speed_error
        self.vars.i_term_previous = i_term
        if k_term + i_term >= 0:
            return k_term + i_term, 0
        else:
            return k_term + i_term, -1 * (k_term + i_term)
    ...
    def get_heading_error(self, waypoints, current_yaw):
        waypoints_dalta_x = waypoints[1][0] - waypoints[0][0]
        waypoints_dalta_y = waypoints[1][1] - waypoints[0][1]
        waypoints_heading = np.arctan(waypoints_dalta_y/waypoints_dalta_x)
        # Return the tuple (x//y, x%y).  Invariant: div*y + mod == x.
        heading_error_mod = divmod((waypoints_heading - current_yaw), np.pi)[1]
    ...
    def get_cte_heading_error(self, v):
        ...
        cte_heading_error = np.arctan(proportional_cte_error/abs(v+0.0001))
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            self.set_brake(brake_output/2)        # in percent (0 to 1)
